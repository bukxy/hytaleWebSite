...
const columns: TableColumn<VoteWebsite>[] = [
...
export default function Vote({ data, votes_websites }: { data: Resource<VoteWebsite> | null; votes_websites: ResourceCollection<VoteWebsite> }) {
    const itemData = data?.data ?? null;
    const itemId = itemData?.id;

    const breadcrumbs: BreadcrumbItem[] = [
        {
            title: itemId ? `Edit vote website` : 'New vote website',
            href: voteWebsite.add().url,
        },
    ];

    const initData = (itemData?: VoteWebsite): VoteWebsiteForm => ({
        name: itemData?.name ?? '',
        url: itemData?.url ?? '',
        is_enabled: itemData?.is_enabled ?? false,
        has_verification: itemData?.has_verification ?? false,
        verification_key: itemData?.verification_key ?? '',
        logo: null,
    });

    const [formData, setFormData] = useState<VoteWebsiteForm>(() => initData(itemData ?? undefined));
    ...
    const formConfig =
        itemId !== undefined ? voteWebsite.editStore.form({ id: itemId }) : voteWebsite.addStore.form();
        ...
        <>
            ...
            <div>
                <div className="rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border">
                    <Heading title={itemId ? `${breadcrumbs[0].title} : ${itemData?.name}` : breadcrumbs[0].title} />
                        ...
                        >
                            {({ processing, errors }) => (
                                <>
                                    ...
                                    <div className="col-span-2 col-start-3 grid gap-2">
                                        ...
                                        {itemId != null && itemData?.logo && (
                                            <div className="grid w-full grid-cols-2 items-center justify-center">
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            if (confirm('Are you sure to delete the logo ?')) {
                                                                router.delete(voteWebsite.deleteLogo(itemId).url);
                                                            }
                                                        }}
                                                    >
                                                ...
                                                <img
                                                    alt={String(itemData.logo.path)}
                                                    src={String(itemData.logo.path)}
                                    ...
                                    >
                                        {processing && <Spinner />}
                                        {itemId ? 'S A V E' : 'C R E A T E'}
                                    </Button>
                                    <InputError message={errors.voteWebsite} />
                                </>
                            )}
                        </Form>
